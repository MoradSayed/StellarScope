...
class Font:
    def __init__(self):
        ...
        self.letterTexCoords = {
            ...
            '+':  (17.0 * w, 1.0 - 17.0 * h + 2 * heightOffset, w - margin, h - margin)    # (925, 1065, 20, 20)
        }
        ...
        thepath= os.path.join("Assets", "Pages", "Engine_gl", "gfx")
        with Image.open(os.path.join(thepath, "Inconsolata_2.png"), mode = "r") as img:
...
class TextLine:
    ...
    def build_text(self, new_text, font:Font, draw_crosshair=True):
        self.vertices = []
        self.vertex_count = 0
        ...
        for i,letter in enumerate(new_text):
            ...
            self.append_vertex(bounding_box, margin_adjustment, i)
            self.vertex_count += 6

        if draw_crosshair:
            bounding_box  = font.get_bounding_box("+")
            self.append_vertex(bounding_box, margin_adjustment, 0, (0.011, 0.008))  # perfect position for the crosshair
            self.vertex_count += 6
    ...
    def append_vertex(self, bounding_box, margin_adjustment, i, custom_start_pos=None):
        if custom_start_pos:
            start_position = custom_start_pos
        else:
            start_position = self.start_position

        #top left
        self.vertices.append(
            start_position[0] - self.letter_size[0] + ((2 + margin_adjustment) * i * self.letter_size[0])
        )
        self.vertices.append(start_position[1] + self.letter_size[1])
